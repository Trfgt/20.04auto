...
template <typename PointT>
static void piROIFilter(float radius_min,
                            float radius_max,
                            float z_limit_min,
                            float z_limit_max,
                            typename pcl::PointCloud<PointT>::Ptr cloud) {
    if (cloud->size()) {
        ...
        *cloud_in = *cloud;
        ...
        for (size_t pt = 0u; pt < cloud_in->size(); ++pt) {
            ...
            if (point.z > z_limit_min && point.z < z_limit_max) {
                if (point.x < forward && point.x > back) {
                    if (point.x > point.y && point.y >= 0.0) {
                        cloud->points.push_back(point);
                    }
                    else if (point.x > -point.y && point.y < 0.0) {
                        cloud->points.push_back(point);
                    }
                    else if (point.y > left && point.y < right) {
                        cloud->points.push_back(point);
                    }
                }
            }
...
static void applyROIFilter(const ROIParams& params,
    ...
    if (params.type == "Cylinder") {
      ...
      else if (params.type == "Pi") {
        piROIFilter<PointI>(roi_radius_min, roi_radius_max, roi_z_limit_min,
                                roi_z_limit_max, cloud);
    } else {
        cylinderROIFilter<PointI>(roi_radius_min, roi_radius_max,
                                  roi_z_limit_min, roi_z_limit_max, cloud);
