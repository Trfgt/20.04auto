    ...
    def lidar_callback(self, _data):
        ...
        for i, obj in enumerate(obj):
            ...
            if(self.State=="Rubber_cone_drive"):
                ...
                # if(self.first==False and 2<bbox_center_x<7 and -1.2<bbox_center_y<1): # 오른쪽 주행
                if(self.first==False and 2<bbox_center_x<5 and -1<bbox_center_y<1.2): # 왼쪽 주행 
                    if 0.03<bbox_width<0.2 and 0.1<bbox_height<0.5:
                    ...
                    # if(0.4<bbox_center_x<7 and -1.2<bbox_center_y<4 and 0.03<bbox_width<0.2 and 0.1<bbox_height<0.5):
                    if(0.4<bbox_center_x<7 and 0.03<bbox_width<0.2 and 0.1<bbox_height<0.5):
                        # print("2"*20)
                        uturn_cone.append((d,bbox_center_x,bbox_center_y))
            ###############################################
        ...
        if(self.State=="Rubber_cone_drive"):
            ...
            if(len(uturn_cone)>0):
                ...
                # mid_point=(lane_point[0],lane_point[1]-3.)
                mid_point=(lane_point[0],lane_point[1]+3)
            ...
            if(mid_point is not None):
                ...
                self.publish_obstacles(lane_point, self.lane_point_pub, color=(1.0, 0.0, 0.0))  
        ###############################################################################
