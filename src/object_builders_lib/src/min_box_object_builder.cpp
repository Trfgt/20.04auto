...
void MinBoxObjectBuilder::computePolygon2dxy(ObjectPtr obj) {
    ...
    //std::cout<< "trytomakeonehorn" << std::endl;
    pcl::NormalEstimation<pcl::PointXYZI, pcl::Normal> ne; // PointXYZI로 수정
    pcl::PointCloud<pcl::Normal>::Ptr cloud_normals(new pcl::PointCloud<pcl::Normal>());
    pcl::search::KdTree<pcl::PointXYZI>::Ptr tree(new pcl::search::KdTree<pcl::PointXYZI>()); // PointXYZI로 수정
    ...
    // RANSAC을 이용한 원뿔 검출
    pcl::SACSegmentationFromNormals<pcl::PointXYZI, pcl::Normal> seg; // PointXYZI로 수정
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_cone(new pcl::PointCloud<pcl::PointXYZI>()); // PointXYZI로 수정
    pcl::ExtractIndices<pcl::PointXYZI> extract; // PointXYZI로 수정
    ...
    if (inliers_cone->indices.size() != 0) {
        double cone_height = coefficients_cone->values[3];
        double cone_angle = coefficients_cone->values[7];
        ...
        PointICloudPtr base_cloud(new PointICloud);
        extract.filter(*base_cloud);
        ...
        pcl::PointXYZI base_center;
        centroid.get(base_center);

        // 가장 높은 점과 밑면 중심 간의 거리 계산
        Eigen::Vector3f cone_apex(coefficients_cone->values[0], coefficients_cone->values[1], coefficients_cone->values[2]);
        double distance = std::sqrt(std::pow(cone_apex[0] - base_center.x, 2) + std::pow(cone_apex[1] - base_center.y, 2));

        if (cone_height <= 0.75 && cone_angle >= 10.0 && cone_angle <= 30.0 && distance < 0.15) { // 거리 조건 추가
            // 라바콘으로 처리하기 위해 폴리곤 설정 및 반환
            obj->height = static_cast<double>(1.1111111111);
            obj->polygon.points.resize(4);
            obj->polygon.points[0].x = static_cast<double>(min_pt[0]);
            obj->polygon.points[0].y = static_cast<double>(min_pt[1]);
            obj->polygon.points[0].z = static_cast<double>(1.1111111111);

            obj->polygon.points[1].x = static_cast<double>(min_pt[0]);
            obj->polygon.points[1].y = static_cast<double>(max_pt[1]);
            obj->polygon.points[1].z = static_cast<double>(1.1111111111);
   
            obj->polygon.points[2].x = static_cast<double>(max_pt[0]);
            obj->polygon.points[2].y = static_cast<double>(max_pt[1]);
            obj->polygon.points[2].z = static_cast<double>(1.1111111111);

            obj->polygon.points[3].x = static_cast<double>(max_pt[0]);
            obj->polygon.points[3].y = static_cast<double>(min_pt[1]);
            obj->polygon.points[3].z = static_cast<double>(1.1111111111);
