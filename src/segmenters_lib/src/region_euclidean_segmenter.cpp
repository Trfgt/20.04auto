...
void RegionEuclideanSegmenter::segment(
    ...
    *cloud = cloud_in;
    ...
    for (size_t pt = 0u; pt < cloud->size(); ++pt) {
        ...
        // E_STOP
        /*if (cloud->points[pt].x <= 6.0 + car_vel/5 && cloud->points[pt].x >= 1. && cloud->points[pt].y < 0.9 && cloud->points[pt].y > -0.9 && cloud->points[pt].z < 1.0 && isFirst) {
            ROS_WARN_STREAM("Object in " << (4 + car_vel/5) << "m");

			//TODO set brakeVal from ros param

			std_msgs::Int16 brakeVal;
            if (car_vel*car_vel > (32767-params_.brake_value)/12){
                brakeVal.data = 30000;
            }
            else{
			brakeVal.data = params_.brake_value + car_vel*car_vel*12;
            }
            ROS_FATAL_STREAM("E-STOP Publishing Actuator with " << brakeVal.data);
