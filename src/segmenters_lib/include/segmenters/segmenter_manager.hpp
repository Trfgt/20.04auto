...
static std::unique_ptr<BaseSegmenter> createNonGroundSegmenter(
    ...
    //2024 추가
    } else if (params.segmenter_type == "DBSCAN") {
        segmenter = std::unique_ptr<BaseSegmenter>(new DBSCAN(params));
        ROS_INFO("[segment] Instance of DBSCAN Non-ground Segmenter created.");
    } else {
